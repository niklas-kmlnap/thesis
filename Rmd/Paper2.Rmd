...
Check baseline model. Hvordan skal jeg tolke modellen jeg tester? Hva representerer variablene jeg putter inn i modellen, og hva representerer y-verdien? 

MLM: TPACK~factor(studiested) er lik OLS: TPACK~factor(studiested)

MLM: TPACK~1 er ikke lik OLS: TPACK~factor(studiested)

Hva er rett måte å sette opp en baseline model på i OLS? (med lm_robust og clusters)

Skal studiested være med som variabel i likningen (fixed effect), eller bare som en cluster (random effect)? 

Beregner jeg individuell TPACK eller studiested TPACK? TPACK for den enkelte PST, eller TPACK for hvert studiested? 

Hvis studiested er med som variable, beregner jeg TPACK per studiested, ikke student? Hvis studiested ikke er med som variabel, regner jeg TPACK for den enkelte student?
...
stolpediagram <- ggplot(dc, aes(x = studiested2, y = var, fill = studiested2)) +
  ...
  scale_fill_manual(values=c("#812244", "#3fabb5", "#bee2e9")) +
...
#ggsave("../output/TPACK_stolpediagram_new.png")
```

# Multiple regression analysis

The purpose of the multiple regression analysis is to explain the variation in the data. I therefore use hierarchical regression [@howitt_introduction_2014; @kelley_multiple_2013] and add the variables step-by-step to examine changes in R2 (explained variance). 
...
interaction.plot(
  x.factor = factor(data_clean$Gender),
  trace.factor = factor(data_clean$studiested), # label=c("institute 1", "institute 2", "institute 3", "institute 4", "institute 5")),
  response = data_clean$TPACK,
  type = "b",
  ylab = "TPACK",
...
# kovariater; we use scale to standardize when using interaction effects
# sjekk: er det riktig å skalere?
model_TK_baseline <- lm_robust(TK ~ 1, #factor(studiested), 
                clusters = factor(studiested), data=data_clean)
...
```{r}
hierarchical_fstat <- function(R_qr, R_q, df_num, df_den) {
  # calculate fstat
  # p. 92 in Kelley & Bolin (2013)
  # numerator df = r; r = diff antall variable
  # denominator df = N - q - r - 1; N = sample size, q = antall variable i enkleste modell

  fstat <- ((R_qr - R_q) / df_num) / ((1 - R_qr) / df_den)
  p     <- pf(fstat, df_num, df_den, lower.tail=F)  
  
  return(p)
}

df_num <- 4 + length(model_TK_level1$term) - length(model_TK_baseline$term)
df_den <- 4 + length(data_clean$TK) - length(model_TK_level1$term) - 1
R_qr   <- model_TK_level1$adj.r.squared
R_q    <- model_TK_baseline$adj.r.squared

hierarchical_fstat(R_qr, R_q, df_num, df_den)
```
...
```{r}
df_num <- 4 + length(model_PCK_level1$term) - length(model_PCK_baseline$term)
df_den <- 4 + length(data_clean$PCK) - length(model_PCK_level1$term) - 1
R_qr   <- model_PCK_level1$adj.r.squared
R_q    <- model_PCK_baseline$adj.r.squared
...
model_TPACK_baseline <- lm_robust(
  TPACK ~ 
    1, #factor(studiested),                       # level 2
  clusters = factor(studiested), data = data_clean
)
summary(model_TPACK_baseline)
```

test: baseline OLS er ikke lik baseline MLM. Hvorfor ikke?

```{r level2_main_test, eval=F}
# ref: https://www.r-bloggers.com/2021/05/clustered-standard-errors-with-r/ [5.9.2024]

# studiested; Institute1 velges som referansekategori siden den har lavest TPACK-verdi av alle
model_TPACK_baseline_test <- lm_robust(
  TPACK ~ 1,                       # level 2
  clusters = factor(studiested), 
  fixed_effects = ~factor(studiested),
  data = data_clean
)
summary(model_TPACK_baseline_test)
```
...
```{r}
df_num <- 4 + length(model_TPACK_level1$term) - length(model_TPACK_baseline$term)
df_den <- 4 + length(data_clean$TPACK) - length(model_TPACK_level1$term) - 1
R_qr   <- model_TPACK_level1$adj.r.squared
R_q    <- model_TPACK_baseline$adj.r.squared
...
```{r}
df_num <- 4 + length(model_TPACK_PCK$term) - length(model_TPACK_level1$term)
df_den <- 4 + length(data_clean$TPACK) - length(model_TPACK_PCK$term) - 1
R_qr   <- model_TPACK_PCK$adj.r.squared
R_q    <- model_TPACK_level1$adj.r.squared
...
```{r}
df_num <- 4 + length(model_TPACK_TK$term) - length(model_TPACK_PCK$term)
df_den <- 4 + length(data_clean$TPACK) - length(model_TPACK_TK$term) - 1
R_qr   <- model_TPACK_TK$adj.r.squared
R_q    <- model_TPACK_PCK$adj.r.squared
...
"all level 1 predictors were centered at the group mean, and all level 2 predictors were centered at the grand mean" (p. 192) (Osborne and Neupert in Teo, 2013)

level 1 predictors: properties related to the PST
level 2 predictors: properties related to the TEI
...
table_variance <- data.frame(
  c(
    nu_e_baseline,
    (nu_e_baseline - nu_e_level1) / nu_e_baseline,
    ((nu_e_baseline - nu_e_PCK) / nu_e_baseline) - ((nu_e_baseline - nu_e_level1) / nu_e_baseline),
    ((nu_e_baseline - nu_e_TK) / nu_e_baseline) - ((nu_e_baseline - nu_e_PCK) / nu_e_baseline) - ((nu_e_baseline - nu_e_level1) / nu_e_baseline)
  ),
  c(
    nu_00_baseline,
    (nu_00_baseline - nu_00_level1) / nu_00_baseline,
    ((nu_00_baseline - nu_00_PCK) / nu_00_baseline) - ((nu_00_baseline - nu_00_level1) / nu_00_baseline),
    ((nu_00_baseline - nu_00_TK) / nu_00_baseline) - ((nu_00_baseline - nu_00_PCK) / nu_00_baseline) + ((nu_00_baseline - nu_00_level1) / nu_00_baseline)
  ),
