...
### screening
library("corrplot")                         # multicollinearity library(car)                                # vif       
library("olsrr")                            # vif
...
```{r}
do_screening <- 0 # skip screening during testing
```
...
"It is recommended to carry out a preliminary analysis of the metric
...
researcher should decide whether to eliminate certain items" (Izquierdo et al, 2014)

Analyze and report the mean, standard deviation, and item-test correlation of each one of the items, as well as the Cronbach’s alpha of the scales of the test
...
print_starvalue <- function (pvalue, effectsize, bonferroni=1) {
  ...
  ifelse (pvalue < 0.05/bonferroni, star <- "*",
          star <- "")))
  return (paste(format(effectsize,digits=antall_desimaler,nsmall=antall_desimaler), star, sep="")) # antall desimaler=2
}
...
create_matrix <- function (df, method="pearson") {
  ...
  for (var1 in df[1:ncol(df)-1]) {
    ...
    for (var2 in df[i:ncol(df)]) {
      ...
      est_p <- print_starvalue(cor_var1_var2$p.value, cor_var1_var2$estimate)
      right_side <- cbind(right_side,est_p) # add column
    }
    
    # add left-side of matrix: covariance
    #left_side <- data.frame(matrix(ncol = i-1, nrow = 1))
    left_side <- format(cov_matrix[i-1,1:i-1],digits=antall_desimaler)
  ...
  # add final row
  #final_row <- data.frame(matrix(ncol = ncol(df), nrow = 1))
  ...
  row_mean <- round(colMeans(df, na.rm=TRUE), digits=antall_desimaler)
  row_sd <- format(psych::SD(df), digits=antall_desimaler)
...
```{r mean_sd_spring}
# velg ut data
df <- data_TPACK_pilot_minuspost[,colnames(data_TPACK_pilot_minuspost) %in% unlist(pilot_list)]

# fjern items der over 1/2 av respondentene mangler NA?
df_noNA <- df[ , colSums(is.na(df)) < nrow(df)/2] 
```

```{r correlation_matrix, eval=FALSE}
pander(create_matrix(df_noNA)) # ordered=FALSE
```

Fjerne TK4, TK7, CK4, TCK2, PK4, PK5, PCK1,
fordi disse korrelerer svakt med sine meditems (ikke signifikant).
TK4 har også signifikant lavere gjennomsnitt enn de andre TK-itemsene. (noe kanskje også TK8 har?)
...
Formålet er å sjekke om et item svarer godt til testresultatet sett over ett, f.eks. hvilke TK-spørsmål som best predikerer TK. 
...
```{r item_test_spring, warning=FALSE}
...
TCK2 har lav item-test correlation. 

#### Reliability: Cronbachs alpha

```{r}
# hvis grupperingen i prinsippet er mulig å skille fra hverandre, så kan Cronbachs alpha brukes til å vurdere likheten mellom gruppene (reliabilitet)
# alpha beregnes per construct, altså TK, TCK, TPACK, osv.
#psych::alpha(data.frame(pre,post,anon,trelis_q))
```

Assumptions: uni-dimensionality (one factor = factor analysis), tau-equality (equal covariance) (ref Revelle, 2019)
...
Fjerner items basert på preTK4 har lav korrelasjon med de andre.

```{r}
# remove columns

# 25.5.2023
dTp_mod <- within(data_TPACK_pilot_minuspost, rm("TK7","CK4")) # "PK2", "TK3","TK8",

#   data, since items removed above
# velg ut data
df <- dTp_mod[,colnames(dTp_mod) %in% unlist(pilot_list)]
...
```{r}
# KMO og Bartlett
KMO_base <- KMO(df_noNA) # addition of PCK1 changed KMO from 0.5 to 0.36
bartlett_base <- cortest.bartlett(df_noNA, n=5,diag=TRUE)
```
...
parallel_base <- fa.parallel(df_noNA,fm='mle',fa='fa',SMC=TRUE,  use="complete.obs") #, )correct=0,cor="poly",
...
factor_model1 <- fa(df_noNA, nfactors = 2,rotate = "oblimin", fm="mle", scores="tenBerge") # missing=TRUE,impute="mean", ,oblique.scores=TRUE, cor="poly", correct=0 covar=TRUE,

print.psych(fa.sort(factor_model1))
```
...
# bruk polykorisk korrelasjon (Flora et al, 2012)
# use covariance matrix (correlation matrix fails because of missing)
factor_model1 <- fa(df_noNA, nfactors = 3,rotate = "oblimin", fm="mle", scores="tenBerge") # missing=TRUE,impute="mean", covar=TRUE,,oblique.scores=TRUE, cor="poly", correct=0
...
# bruk polykorisk korrelasjon (Flora et al, 2012)
# use covariance matrix (correlation matrix fails because of missing)
factor_model1 <- fa(df_noNA, nfactors = 4,rotate = "oblimin", fm="mle", scores="tenBerge") # missing=TRUE,impute="mean", covar=TRUE,,oblique.scores=TRUE, cor="poly", correct=0
...
print(factor_model1$communality[factor_model1$communality<0.6])
```

```{r}
# fjern items med lav communality; remove columns
#df_mod <- within(df_noNA, rm("PK5"))

# 26.5.2023, remove cross-loadings
df_mod <- within(df_noNA, rm("TK5"))
```
...
```{r}
# KMO og Bartlett
KMO_base <- KMO(df_mod) # addition of PCK1 changed KMO from 0.5 to 0.36
bartlett_base <- cortest.bartlett(df_mod, n=4,diag=TRUE)
```
...
parallel_base <- fa.parallel(df_mod,fm='mle',fa='fa',SMC=TRUE) #, ),  correct=0,cor="poly",use="complete.obs"
#parallel <- fa.parallel(df,fm='alpha',fa='fa',nfactors=7,use="complete.obs",cor='poly', error.bars=TRUE, SMC=TRUE) # complete.obs = delete NA; poly for fa recommended av Revelle & Condon, 2019
...
```{r}
factor_model2 <- fa(df_mod, nfactors = 2, rotate = "oblimin", fm="mle", scores="tenBerge") # ,oblique.scores=TRUE, cor="poly", correct=0

print.psych(fa.sort(factor_model2))
```
...
# p = .8 (Cohen, 1990)
factor_model2 <- fa(df_mod, nfactors = 3, rotate = "oblimin", fm="mle", scores="tenBerge") #, missing=TRUE, impute="mean") # n.iter=1000, p=.8,  #,oblique.scores=TRUE, cor="poly", correct=0)
...
```{r}
factor_model2 <- fa(df_mod, nfactors = 4, rotate = "equamax", fm="uls", scores="tenBerge") #,oblique.scores=TRUE, cor="poly", correct=0)
...
order2 <- factor_model2$Phi # 2nd order factors
print(order2)

# eigenvalues
print(factor_model2$values)              

# loadings
print.psych(factor_model2) #, cut=c)
```
...
print(factor_model2$communality[factor_model2$communality>0.6])
# dette er standardized loadings ??
#print(efa7_ml_pro$communalities[efa7_ml_pro$communalities>0.6])

# avoid below 0.4
print(factor_model2$communality[factor_model2$communality<0.4])
```
...
print(factor_model2$Structure, cutoff=0.8)

factor_model2$Structure
...
```{r}
round(factor_model2$residual, digits=2)
```

```{r}
# check reliability
#cronbach.alpha(df_mod, na.rm=TRUE)

# tegn factor_model
stimodel <- fa.diagram(factor_model2)
#print(stimodel)
```

```{r}
# compare with intervall pearson (appx normal distributed)
pander(create_matrix(df_mod))
```


### McDonalds omega

EFA som forberedelse til omega (informal test)
...
```{r eval=T}
alpha(df_mod[,c("TK1", "TK2", "TK3")])
alpha(df_mod[,c("CK1", "PCK2", "PCK5", "CK5", "CK3", "CK2")])
alpha(df_mod[,c("TPK1","TPK4","TCK1","TCK4", "TPACK2","TPACK4")]) # , "TPK4"
```

```{r}
omega(df_mod, fm="mle") #, rotate="Promax")
```
...
* treat the data as interval or ordinal? (Watkins, 2018)
* missing data: use imputation (McNeish, 2017; Schafer and Graham, 2002)
* linearity-check: scatterplot, collinearity (Flora et al., 2012)
* normal or nonnormal distribution? Normality is important for test statistic. For nonnormal use Satorra-Bentler scaled chi-square and robust SE (Flora et al., 2012; McNeish, 2017). Nonnormal means skew>2 and kurtosis>9 (ref?). 
* aim for approximate fit, not exact fit (Flora et al., 2012; Weston and Gore, 2006)
* item selection: communalities (h2) - high (0.6 < h2 > 0.8) or wide (0.2 < h2 > 0.8)

"The swain and FAiR packages in R came perform the Swain correction and the FAiR package can also compute the Bartlett correction" 

"Swain correction tends to provide the smallest correction but provides researchers with the most power to detect whether their model fits poorly"

yuan.bentler forutsetter adf-estimering. (McNeish, 2017) som krever at utvalgsstørrelser er minst p(p+1)/2, der p er antall items i modellen. Med p=12 blir det 78 respondenter, som er mer enn jeg har (p=106).

test="satorra.bentler"  # for robust test når ikke-normalfordelte data
...
Hensikt: bekrefte measurement model, altså indikatorenes kobling til de latente faktorene. (ser ikke på korrelasjoner her?)
...
TPACK_PST_0 <- cfa(cfaspec, data= cfadata, std.lv=TRUE,
             ...
             # DWLS URL: https://lavaan.ugent.be/tutorial/est.html
             estimator="ML", #missing="pairwise", 
             #estimator="WLS",# missing="ml",
             #estimator="ULS", 
             #ordered=TRUE,
             ...
             #se="boot", bootstrap=1000, # 10000 parallel=
             #parallel = "snow", ncpus = 4) # ref: https://github.com/yrosseel/lavaan/issues/23
...
# The argument standardized = TRUE augments the output with standardized parameter values. Two extra columns of standardized parameter values are printed. In the first column (labeled Std.lv), only the latent variables are standardized. In the second column (labeled Std.all), both latent and observed variables are standardized. The latter is often called the ‘completely standardized solution’. (ref: https://lavaan.ugent.be/tutorial/sem.html)
  ...
  # ref Rönkkö and Cho (2022)
#discriminantValidity(TPACK_PST_1, merge=TRUE)
# reliability, ref.: https://groups.google.com/g/lavaan/c/tPEPXQqQR48
#semTools::reliability(TPACK_PST_1)                         # deprecated
  print("reliability")
...
```{r estimate_9}
# egenfaktor
model9 <- do_cfa(tinker2, CFA_TPACK_PST)
rel9 <- print_model(model9)
...
```{r estimate_10}
# egenfaktor
model10 <- do_cfa(pseudo_1faktor, CFA_TPACK_PST)
rel10 <- print_model(model10)
...
kolonnenavn <- c("AIC", "chi-square", "df", "p-value","RMSEA",
                 "RMSEA (90%) CI lower","RMSEA (90%) CI upper", 
                 "CFI","TLI", "SRMR") #, "chi2/df", 
                 #"chi-square scaled Swain", "p-value Swain", 
                 #"swain chi2/df")
anbefalte_verdier <- c("x", "x", "x", ">0.05", "<0.06(0.05-0.10)", "x", "x", ">0.95(0.9-0.95)", ">0.95", "<0.08(0.08-0.15)") #, "<2", "x", ">0.05", "<2") # Weston and Gore (2006); Gallagher and Brown (2013)

# skal jeg bruke scaled eller robust?
# bruk tre siffer for å kunne skille mellom CFI på 0.002 (discriminatory validity, ref. Shaffer et al. (2016))

#                  fitMeasures(TPACK_PST_0,"chisq.scaled")/
#                    fitMeasures(TPACK_PST_0,"df.scaled"),
#                80.35, 0.002, 80.35/fitMeasures(TPACK_PST_0,"df.scaled")),3)


fit9 <- round(fitMeasures(model9,
                  c("aic","chisq.scaled", "df.scaled",
                    "pvalue.scaled","rmsea.robust",
                    "rmsea.ci.lower.robust","rmsea.ci.upper.robust",
                    "cfi.robust","tli.robust","srmr")),3)
fit10 <- round(fitMeasures(model10,
                  c("aic","chisq.scaled", "df.scaled",
                    "pvalue.scaled","rmsea.robust",
                    "rmsea.ci.lower.robust","rmsea.ci.upper.robust",
                    "cfi.robust","tli.robust","srmr")),3)

radnavn <- c("anbefalt", "tinker2", "pseudo_1faktor")

fit_table <- rbind(anbefalte_verdier,fit9)
fit_table <- rbind(fit_table, fit10)

colnames(fit_table) <- kolonnenavn
row.names(fit_table) <- radnavn
...
#CFA_TPACK_PST <- CFA_TPACK_PST - 3 # flytt nullpunkt
...
df <- CFA_TPACK_PST[,unlist(TPACK_PST_items[1:3])]
describe(df)      # with missing (see n)? (na.rm=TRUE)
score_TPACK <- scoreItems(TPACK_PST_items,df)
#print(score_TPACK,option=FALSE)
...
```{r omega_mle4}
omega0 <- omegaFromSem(model10)
omega0
#print(c(omega1$alpha, omega1$omega.tot)

# get correlation matrix of observed variables
cor_mat <- lavInspect(model10, what="cor.ov")
omega(cor_mat, nfactors=3)
...
```{r eval=T}
#measureQ(model9, CFA_TPACK_PST)

measureQ(model10, CFA_TPACK_PST)
```
